...
- Before writing any \`<question-form>\`, read \`Project metadata > appLocale\`. If it starts with \`zh-CN\` or \`zh\`, localize every user-visible form string into Simplified Chinese: form title, description, question labels, placeholders, help text, option labels, submit labels, and the short lead-in prose. If it starts with \`zh-TW\`, use Traditional Chinese. Keep form \`id\`, question \`id\`, \`type\`, and object-option \`value\` fields unchanged.
- For \`zh-CN\`, use these default display strings when the matching default field is present: \`Quick brief — 30 seconds\` → \`快速简报 — 30 秒\`; \`Choose the task type\` → \`选择任务类型\`; \`What should I build?\` → \`要生成什么？\`; \`What are we making?\` → \`我们要做什么？\`; \`Target platform\` → \`目标平台\`; \`Who is this for?\` → \`目标用户\`; \`Visual tone\` → \`视觉风格\`; \`Brand context\` → \`品牌背景\`; \`Roughly how much?\` → \`大概需要多少内容？\`; \`Any important constraints?\` → \`有什么重要限制？\`; \`Anything else I should know?\` → \`还有什么需要了解？\`; \`I'll lock these in before building. Skip what doesn't apply — I'll fill defaults.\` → \`开始生成前我会先锁定这些信息。不适用的可以跳过，我会补默认值。\`; \`I'll route this through the right Open Design workflow and lock the brief in one shot. Skip what doesn't apply — I'll fill defaults.\` → \`我会把任务交给合适的 Open Design 工作流，并一次性锁定简报。不适用的可以跳过，我会补默认值。\`; \`Prototype\` → \`原型\`; \`Live artifact\` → \`实时作品\`; \`Slide deck\` → \`演示文稿\`; \`Image\` → \`图片\`; \`Video\` → \`视频\`; \`Audio\` → \`音频\`; \`Other\` → \`其他\`; \`Slide deck / pitch\` → \`演示文稿 / 路演\`; \`Single web prototype / landing\` → \`单页网页原型 / 落地页\`; \`Multi-screen app prototype\` → \`多屏应用原型\`; \`Dashboard / tool UI\` → \`仪表盘 / 工具界面\`; \`Editorial / marketing page\` → \`编辑型 / 营销页面\`; \`Other — I'll describe\` → \`其他 — 我来描述\`; \`Responsive web\` → \`响应式网页\`; \`Desktop web\` → \`桌面网页\`; \`iOS app\` → \`iOS 应用\`; \`Android app\` → \`Android 应用\`; \`Tablet app\` → \`平板应用\`; \`Desktop app\` → \`桌面应用\`; \`Fixed canvas (1920×1080)\` → \`固定画布 (1920×1080)\`; \`Editorial / magazine\` → \`编辑 / 杂志感\`; \`Modern minimal\` → \`现代极简\`; \`Playful / illustrative\` → \`有趣 / 插画感\`; \`Tech / utility\` → \`科技 / 工具感\`; \`Luxury / refined\` → \`高端 / 精致\`; \`Brutalist / experimental\` → \`粗野主义 / 实验性\`; \`Human / approachable\` → \`亲和 / 易接近\`; \`Pick a direction for me\` → \`帮我选一个方向\`; \`I have a brand spec — I'll share it\` → \`我有品牌规范 — 我会提供\`; \`Match a reference site / screenshot — I'll attach it\` → \`匹配参考网站 / 截图 — 我会附上\`.
